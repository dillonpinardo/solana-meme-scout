...
for (const pair of best) {
  const liquidity=n(pair.liquidity?.usd), volume=n(pair.volume?.h24), buys=n(pair.txns?.h24?.buys), sells=n(pair.txns?.h24?.sells), ratio=buys/Math.max(sells,1), move=n(pair.priceChange?.h24), age=pair.pairCreatedAt?(Date.now()-pair.pairCreatedAt)/3600000:Infinity;
  ...
  const body={username:'Meme Scout',allowed_mentions:{parse:[]},embeds:[{title:'New paper-scan candidate: '+pair.baseToken.symbol,url:pair.url,color:6591999,fields:[{name:'Found at',value:new Date().toLocaleString(),inline:true},{name:'Price found',value:'$'+(pair.priceUsd||'unknown'),inline:true},{name:'Score',value:String(score),inline:true},{name:'Liquidity',value:'$'+Math.round(liquidity).toLocaleString(),inline:true},{name:'24h volume',value:'$'+Math.round(volume).toLocaleString(),inline:true},{name:'Buy/sell',value:ratio.toFixed(2),inline:true},{name:'Token address',value:String(pair.baseToken.address)}],footer:{text:'Paper signal only — verify before any trade.'}}]};
